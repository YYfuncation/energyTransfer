...
@interface NJBannerDetailsViewController ()<UIWebViewDelegate,UITabBarControllerDelegate>
{
    JSContext *_context;
}
...
@property (nonatomic, assign) BOOL isLoginHtml;// 是否是登录页（空白）
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([request.URL.absoluteString containsString:@"member/login"]) {
        self.isLoginHtml = YES;
    } else {
        self.isLoginHtml = NO;
    }
    return YES;
