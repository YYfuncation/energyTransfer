...
{
    NJHomeHotCellCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NJHomeHotCellCollectionViewCell" forIndexPath:indexPath];
    if (indexPath.row == 0) {
        cell.backgroundColor = [UIColor colorWithRed:117.0f/255.0f green:177.0f/255.0f blue:202.0f/255.0f alpha:1];
    }else if (indexPath.row == 1) {
        cell.backgroundColor = [UIColor colorWithRed:136.0f/255.0f green:142.0f/255.0f blue:196.0f/255.0f alpha:1];
    }else if (indexPath.row == 2) {
        cell.backgroundColor = [UIColor colorWithRed:141.0f/255.0f green:181.0f/255.0f blue:213.0f/255.0f alpha:1];
    }else  {
        cell.backgroundColor = [UIColor colorWithRed:141.0f/255.0f green:213.0f/255.0f blue:160.0f/255.0f alpha:1];
    }
    
    NJHomeHotModel * model = _dataArray[indexPath.row];
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:model.iconUrl] placeholderImage:[UIImage imageNamed:@"icon_mainIcon"]];
    cell.titleName.text = model.brand;
    cell.secondTitleName.text = model.titleName;
    BOOL isLogin = [[NSUserDefaults standardUserDefaults] boolForKey:kLoginSuccess];
    if (isLogin) {
        cell.productPrice.text = [NSString stringWithFormat:@"￥%.2f",model.marketPrice];
        cell.productPrice.textColor = [UIColor blackColor];
    }else{
        cell.productPrice.text = @"企业用户可见";
        cell.productPrice.textColor = [UIColor redColor];
    }
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [self.collectionView registerNib:[UINib nibWithNibName:@"NJHomeHotCellCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"NJHomeHotCellCollectionViewCell"];
